...
function renderToPage(locality, street, from, to, where) {
    ...
    fetch(ENDPOINT)
        ...
        .then((data) => {
            ...
            for (let x = 0; x < today.length ; x++) {
                let row = today[x]
                renderToPage(row.locality, row.streets, row.from, row.to , today)
            }

            let future = data.future

            for (let x = 0; x < future.length ; x++) {
                let row = future[x]
                renderToPage(row.locality, row.streets, row.from, row.to , future)
            }
